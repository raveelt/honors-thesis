...
\begin{tabular}{|c|c|c|}	
	...
	Fig. \ref{fig:4-city-circuit} shows us the circuit for our first hamiltonian cycle initialized in eigenstate $|01101100\rangle$. We will run a similiar circuit for the other two hamiltonian cycles with the only change being the eigenstate initialization. 
...
\vspace{0.5cm}
 ...
 We can see in Fig. \ref{fig:4-city-graphic-3} our estimation for the first cycle with the highest counts is $111\; 111$. The left set of binary digits refer to the phase estimated after our constrained is satisfied and similiarly the right is for estimation before. We can convert these binary digits to their decimal representation: 
 
 $$0.111, \; 0.111 = 0.875, \; 0.875$$
 
 We can summarize the most probable results in a table:
...
\begin{table}[ht!]
	...
	\begin{tabular}{lllllll} % Updated for 7 columns
		\toprule
		Cycle & Expected & N & 1st & Prob. & 2nd & Prob. \\
		\midrule
...
\end{align*}
	...
	Because we are dealing with the symmetric case. Based on Table \ref{table:ham-cycle-details-5-city} we can simply use the results and conversions from the first three cycles. Thus we will be estimating phases using the following eigenstates:
	...
	$$ u_{1110}  = e^{i2\pi(0.825)}$$
	$$ u'_{1110} = e^{i2\pi(0.35)}$$
	...
	$$ u_{1202}  = e^{i2\pi(0.75)}$$
	$$ u'_{1202} = e^{i2\pi(0.5)}$$
	...
	$$ u_{1138}  = e^{i2\pi(0.75)}$$
	$$ u'_{1138} = e^{i2\pi(0.5)}$$
	...
	$$ u_{1670}  = e^{i2\pi(0.575)}$$
	$$ u'_{1670} = e^{i2\pi(0.35)}$$
	...
	\end{center}
		...
		$$ u_{1830} =  e^{i2\pi(0.775)}$$
	$$ u'_{1830} =  e^{i2\pi(0.55)}$$
	...
	$$ u_{1726} =  e^{i2\pi(0.55)}$$
	$$ u'_{1726} = e^{i2\pi(0.55)}$$
	...
	$$ u_{2230} =  e^{i2\pi(0.575)}$$
	$$ u'_{2230} = e^{i2\pi(0.35)}$$
	...
	$$ u_{2410}  = e^{i2\pi(0.725)}$$
	$$ u'_{2410}  = e^{i2\pi(0.5)}$$
	...
	\vspace{0.5cm}
		...
		\caption{3 \& 4 qubit phase estimation for the 5-city graph. Hamiltonian cycles: 1 to 3}
		\label{fig:4-city-graphic}
	\end{figure}		
	...
	\begin{figure}[!h]
		...
		\caption{3 \& 4 qubit phase estimation for the 5-city graph. Hamiltonian cycles: 4 to 6}
		\label{fig:4-city-graphic}
	\end{figure}		
	...
	\begin{figure}[!h]
		...
		\caption{3 \& 4 qubit phase estimation for the 5-city graph. Hamiltonian cycles: 7 to 9}
		\label{fig:4-city-graphic}
	\end{figure}		
	...
	\begin{figure}[!h]
		...
		\caption{3 \& 4 qubit phase estimation for the 5-city graph. Hamiltonian cycles: 10 to 12}
		\label{fig:4-city-graphic}
	\end{figure}		
	...
	The code presented here is simulations for the four city . This code can be further extended to larger graphs.
