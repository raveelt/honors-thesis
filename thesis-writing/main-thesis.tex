...
Given $U$'s diagonal nature, its eigenstates align with the basis vectors. Our focus is on specific eigenstates corresponding to the Hamiltonian cycles, determined by the phases. To comprehend how the diagonal elements of $U$ derive from the individual $U_j$ matrices, we visualize the tensor product construction. Notably, the product populates the diagonal elements of $U$ allowing a simplification where we consider these elements directly:
	...
	Now we need to construct the unitary operator and eigenstates but we will first discuss it abstractly that would be applicable to any 4-city graph
	
	\subsection{Unitary Operator and Eigenstates for The 4-city graph}
	For the 4 city problem we will start with four matrices to represent the edgeweights from each node. Using \ref{Uj-elements} we can construct matrix $A$:
	...
	$$\Large U = A \otimes B \otimes C \otimes D$$
...
Thus our eigenstate for \ref{one-ham-cycle-4-city} will be  $|225\rangle$ or $|11100001\rangle$. We can perform an identical process for all the hamiltonian cycles to find their corresponding eigenstates. These are all listed in table \ref{table:ham-cycle-details-4-city}
...
\end{table}
	...
	\subsection{Problem Specific Operator and Eigenstates}
	
	Our matrix U's diagonals will look like the following:
	...
	Because we are dealing with the symmetric case. Based on Table \ref{table:ham-cycle-details-4-city} we can simply 
	
	\section{Results: Simulations with Qiskit}
	
	
	Lets consider the following example of a symmetric 4-city system we are considering the symmetric case thus we only need to look at three hamiltonian cycles. The constraint for our bottleneck solution in this case will involve $\gamma < 6$
...
\begin{tikzpicture}[scale=3, every node/.style={circle, draw, fill=blue!20}]
	...
	\node (4) at (0,1) {A};
	% Edges with weights
	\draw[blue!60, ultra thick] (1) -- (2) node[midway, below, fill=none, draw=none, shape=rectangle] {$5$};
	\draw[blue!60,ultra  thick] (2) -- (3) node[midway, right, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [blue!60, ultra thick](3) -- (4) node[midway, above, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [blue!60,ultra  thick](4) -- (1) node[midway, left, fill=none, draw=none, shape=rectangle] {$4$};
	\draw (1) -- (3) node[pos = 0.4, below, fill=none, draw=none, shape=rectangle] {$6$};
	\draw (2) -- (4) node[pos = 0.6, above, fill=none, draw=none, shape=rectangle] {$2$};
	...
	\node (8) at (2.5,1) {A};
	% Edges with weights
	\draw[black!30!green, ultra thick] (5) -- (6) node[midway, below, fill=none, draw=none, shape=rectangle] {$5$};
	\draw[black!30!green, ultra thick] (6) -- (7) node[midway, right, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [black!30!green, ultra thick](7) -- (8) node[midway, above, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [black!30!green, ultra thick](8) -- (5) node[midway, left, fill=none, draw=none, shape=rectangle] {$4$};
	\draw (5) -- (7) node[pos = 0.4, below, fill=none, draw=none, shape=rectangle] {$0$};
	\draw (6) -- (8) node[pos = 0.6, above, fill=none, draw=none, shape=rectangle] {$2$};
...
\begin{tikzpicture}[scale=3, every node/.style={circle, draw, fill=blue!20}]
	...
	\draw [blue!60, ultra thick](3) -- (4) node[midway, above, fill=none, draw=none, shape=rectangle] {$4$};
	\draw (4) -- (1) node[midway, left, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [blue!60, ultra thick](1) -- (3) node[pos = 0.4, below, fill=none, draw=none, shape=rectangle] {$6$};
	\draw [blue!60, ultra thick](2) -- (4) node[pos = 0.6, above, fill=none, draw=none, shape=rectangle] {$2$};
	...
	\node (8) at (2.5,1) {A};
	% Edges with weights
	\draw[red, ultra thick] (5) -- (6) node[midway, below, fill=none, draw=none, shape=rectangle] {$5$};
	\draw (6) -- (7) node[midway, right, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [red, ultra thick](7) -- (8) node[midway, above, fill=none, draw=none, shape=rectangle] {$4$};
	\draw (8) -- (5) node[midway, left, fill=none, draw=none, shape=rectangle] {$4$};
	\draw  [red, ultra thick](5) -- (7) node[pos = 0.4, below, fill=none, draw=none, shape=rectangle] {$0$};
	\draw  [red,ultra  thick](6) -- (8) node[pos = 0.6, above, fill=none, draw=none, shape=rectangle] {$2$};
...
\begin{tikzpicture}[scale=3, every node/.style={circle, draw, fill=blue!20}]
	...
	\node (8) at (2.5,1) {A};
	% Edges with weights
	\draw (5) -- (6) node[midway, below, fill=none, draw=none, shape=rectangle] {$5$};
	\draw [red,ultra thick](6) -- (7) node[midway, right, fill=none, draw=none, shape=rectangle] {$4$};
	\draw (7) -- (8) node[midway, above, fill=none, draw=none, shape=rectangle] {$4$};
	\draw [red,ultra thick](8) -- (5) node[midway, left, fill=none, draw=none, shape=rectangle] {$4$};
	\draw  [red,ultra thick](5) -- (7) node[pos = 0.4, below, fill=none, draw=none, shape=rectangle] {$0$};
	\draw  [red,ultra thick](6) -- (8) node[pos = 0.6, above, fill=none, draw=none, shape=rectangle] {$2$};
